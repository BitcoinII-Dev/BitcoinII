...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " bitcoinII-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  bitcoinII-tx [options] <hex-tx> [commands]  Update hex-encoded bitcoinII transaction\n"
                "or:     bitcoinII-tx [options] -create [commands]   Create hex-encoded bitcoinII transaction\n"
                "\n";
